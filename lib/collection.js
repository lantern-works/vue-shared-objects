import Item from './item.js';
 class Collection {
    ...
    constructor(id = 'all') {
        ...
        this._node = Collection.ITEMS.get(this._id); // used for watching
    ...
    */
    async remove(item) {
       ...
       item.set('_deleted', true);
       await this._node.get(item.key).get('_deleted').push(true);
    }
    ...
    */
    register(v,k) {
        ...
        try {
            ...
            // watch keys for update
            this._node.get(key).map((v,k) => {
              item.onChange(v,k)
            });
        }
        catch(e) {
